...
# Tested on Genymotion emulator API-26
...
sleep 2
adb shell  "ls -l /sdcard/$CERTNAME"

echo "Mounting /system as writable"
adb shell "su -c 'mount'"|grep -i "/system type"|grep -iv "magisk"
adb shell "su -c 'mount -o rw,remount /system'"
adb shell "su -c 'mount'"|grep -i "/system type"|grep -iv "magisk"


echo "Adding $CERTNAME to the system certs"
adb shell  "su -c 'mv /sdcard/$CERTNAME /system/etc/security/cacerts/'"
adb shell "su -c 'ls -l /system/etc/security/cacerts'"|grep -i $CERTNAME
